    ...
    function appendStudentsList(learnerActivityContent, activityData) {
        ...
        if (learnerActivityContent.length === 0) {
            ...
            for (let i = 0; i < learnerActivityContent.length; i++) {
                const enrollee_id =
                    learnerActivityContent[i]["learner_course_id"];
                const learner_id = learnerActivityContent[i]["learner_id"];
                const learner_fname =
                    learnerActivityContent[i]["learner_fname"];
                const learner_lname =
                    learnerActivityContent[i]["learner_lname"];
    ...
    function reDisplayActivity(activityData, activityCriteriaData) {
        ...
        for (let i = 0; i < activityData.length; i++) {
            ...
            const activity_instructions =
                activityData[i]["activity_instructions"];
            ...
            if (activityData !== null) {
                ...
                <div class="flex flex-row items-center">
                    ...
                    <table class="rounded-xl table table-fixed w-full">
                ...
                if (activityCriteriaData !== null) {
                    for (let x = 0; x < activityCriteriaData.length; x++) {
                        const activity_content_criteria_id =
                            activityCriteriaData[x][
                                "activity_content_criteria_id"
                            ];
                        const activity_content_id =
                            activityCriteriaData[x]["activity_content_id"];
                        const criteria_title =
                            activityCriteriaData[x]["criteria_title"];
                        const score = activityCriteriaData[x]["score"];

                        activityContent_disp += `
                                    <tr>
                                        <td>
                                            <input type="text" class="criteriaTitle_input w-full p-3" value="${criteria_title}" disabled>
                                        </td>
                                        <td>
                                            <input type="text" class="criteriaScore_input flex text-center p-3 w-full" value="${score}" disabled></td>
                                        <td>
                                            ...
                                            <div class="hidden flex flex-col edit_btns w-full space-y-2">
                                            
                                                <button data-activity-content-criteria-id="${activity_content_criteria_id}" data-activity-content-id="${activity_content_id}" class="p-3 mx-2 font-semibold text-white bg-darthmouthgreen rounded-xl saveRowCriteriaBtn hover:bg-green-900">Save</button>

                                                <button data-activity-content-criteria-id="${activity_content_criteria_id}" data-activity-content-id="${activity_content_id}" class="p-3 mx-2 font-semibold text-white bg-red-600 rounded-xl deleteRowCriteriaBtn hover:bg-red-900">Delete</button>

                                                <button class=" p-3 mx-2 font-semibold text-white bg-red-600 rounded-xl cancelRowCriteriaBtn hover:bg-red-900">Cancel</button>
                                            </div>
                                        </td>
                                    </tr>
                                    `;
                ...
                activityContent_disp += `
                    ...
                    <button id="" data-activity-id="${activity_id}" data-activity-content-id="${activity_content_id}" class="addNewCriteria hidden p-3 mx-2 font-semibold text-white bg-darthmouthgreen rounded-xl hover:bg-green-900">Add Criteria</button>
                    <div class="editCriteria_clickedBtn hidden mt-3" id=""> 
                        <button id="" data-activity-id="${activity_id}" data-activity-content-id="${activity_content_id}" class="saveCriteriaBtn text-xl p-3 text-white rounded-xl bg-darthmouthgreen hover:bg-green-900">Save Criteria</button>
                        <button id="" class="cancelCriteriaBtn text-xl p-3 text-white bg-red-600 hover:bg-red-900 rounded-xl">Cancel</button>
                    </div>
                    <br>
                    <br>
                    <br>
                ...
                if (activityCriteriaData !== null) {
                    for (let x = 0; x < activityCriteriaData.length; x++) {
                        ...
                        activityContent_disp += `
                                    <tr>
                                        <td>
                                            <input type="text" class="criteriaTitle_input p-3 w-full" value="${criteria_title}" disabled>
                                        </td>
        ...
        $(".saveInstructionsBtn").on("click", function (e) {
            ...
            const activity_instructions = $(this)
                .closest(".editInstructions_clickedBtn")
                .siblings("textarea.activity_instructions")
                .val();
        ...
        $(".editRowCriteriaBtn").on("click", function (e) {
            ...
            const indexToSave = activityCriteriaData.findIndex(
                (item) =>
                    item.criteria_title === criteriaTitle &&
                    item.score == criteriaScore,
            );

            tr.find(".deleteRowCriteriaBtn").data(
                "array-criteria-index",
                indexToSave,
            );
            tr.find(".saveRowCriteriaBtn").data(
                "array-criteria-index",
                indexToSave,
            );
        ...
        $(".saveRowCriteriaBtn").on("click", function (e) {
            ...
            if (criteria_id !== -1) {
                activityCriteriaData[criteria_id]["criteria_title"] =
                    criteriaTitle;
        ...
        $(".saveCriteriaBtn").on("click", function (e) {
            ...
            for (let i = 0; i < activityCriteriaData.length; i++) {
                const activity_content_id =
                    activityCriteriaData[i]["activity_content_id"];
                const criteria_title =
                    activityCriteriaData[i]["criteria_title"];
        ...
        $(".addNewCriteria").on("click", function (e) {
            ...
            var criteriaTitleInput = $(
                '<input type="text" class="addNewCriteriaTitle flex text-center w-full p-3">',
            );
            newRow.append($("<td>").append(criteriaTitleInput));

            // Create the second cell (td) with an input field for score
            var scoreInput = $(
                '<input type="number" class="addNewCriteriaScore flex text-center w-full p-3">',
            );
            newRow.append($("<td>").append(scoreInput));

            // Create the third cell (td) with Save and Delete buttons
            var saveButton = $(
                '<button class="p-3 mx-2 font-semibold text-white bg-green-600 rounded-xl saveNewCriteriaBtn hover:bg-green-900">Save</button>',
            );
            var deleteButton = $(
                '<button class="p-3 mx-2 font-semibold text-white bg-red-600 rounded-xl deleteNewCriteriaBtn hover:bg-red-900">Delete</button>',
            );
            ...
            $(".saveNewCriteriaBtn").on("click", function () {
                const criteriaTitle = $(this)
                    .closest("tr")
                    .find(".addNewCriteriaTitle")
                    .val();
                const criteriaScore = $(this)
                    .closest("tr")
                    .find(".addNewCriteriaScore")
                    .val();
        ...
        $(".saveTotalScoreBtn").on("click", function (e) {
            ...
            const activityContentID = $(this).data("activity-content-id");
            const activityID = $(this).data("activity-id");
            const activity_totalScore = $(this)
                .closest(".editTotalScore_clickedBtn")
                .siblings("input.overallScore_input")
                .val();
